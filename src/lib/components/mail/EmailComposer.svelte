<script lang="ts">
  ...
  $effect(() => {
    if ($identities.length && !identityId) {
  ...
  const identity = $derived($identities.find(i => i.id === identityId))
  ...
  <div class="composer-toolbar">
    ...
    <div class="toolbar-actions">
      <button
        class="send-btn"
        onclick={handleSend}
        disabled={sending || $mailLoading || !toRaw.trim() || !identityId}
      >
  ...
  <div class="fields">
    {#if $identities.length > 1}
      <div class="field-row">
        ...
        <select id="compose-from" bind:value={identityId}>
          {#each $identities as id}
            <option value={id.id}>{id.name} &lt;{id.email}&gt;</option>
          {/each}
        </select>
      </div>
    {:else if identity}
      <div class="field-row">
        ...
        <span class="field-static">{identity.name} &lt;{identity.email}&gt;</span>
      </div>
    {/if}
